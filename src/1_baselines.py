...
while count < REPETITIONS:
    ...
    for cluster in random.sample(intents, NUM_CLUSTERS):
        ...
        cid += 1
    prompt = prompt + """\nPlease use one or two words to interpret each cluster without explanation. Please provide your response in the following format directly: "interpretation 1\ninterpretation 2\n...". No extra sentences are included.The following responses should start from cluster 1 and end with cluster 24. Here are the interpreted clusters separated by a new line (do not repeat or paraphrase this sentence):\n"""

    response = ollama.chat(model=LLM_MODEL, messages=[{'role': 'user', 'content': prompt}], keep_alive=0)['message']['content']
...
res.to_csv("output/baselines_{llm}_metrics_{x}_{y}.csv".format(llm=LLM_MODEL, x=NUM_CLUSTERS, y=NUM_SAMPLES_PER_CLUSTER), index=False)
