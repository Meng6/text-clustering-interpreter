...
def get_llms_response(dataset, num_clusters, num_samples_per_cluster, llm_model):
    ...
    for cluster in random.sample(intents, num_clusters):
        ...
        cid += 1
    prompt = prompt + """
There are two experts who interpret these clusters differently, one is a musician, the other is a data scientist.
Please mimic their conversation and try to reach agreement at the end. Only return the agreement without explanation in the format of "cluster number: interpretation of the cluster". Use one or two words to interpret each cluster.
"""

    response = ollama.chat(model=llm_model, messages=[
        {'role': 'user', 'content': prompt},
        {'role': 'user', 'content': 'Extract each cluster label in order after reaching the agreement. Please provide your response in the following format directly: "interpretation 1\ninterpretation 2\n...". No extra sentences are included. The following responses should start from cluster 1 and end with cluster 24. Here are the interpreted clusters separated by a new line (do not repeat or paraphrase this sentence):\n'}
        ])['message']['content']
...
res.to_csv("output/musician_ds_onemodel_{llm}_metrics_{x}_{y}.csv".format(llm=LLM_MODEL, x=NUM_CLUSTERS, y=NUM_SAMPLES_PER_CLUSTER), index=False)
